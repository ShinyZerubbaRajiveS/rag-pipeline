...
#     def retrieve(question: str, top_k: int = 5, source_filter: str = None) -> list:
#         if collection.count() == 0:
#             print("Database is empty! Run store_chunks() first.")
#             return []
...
#         # build where clause if source filter provided
...
#             n_results        = top_k,
#             include          = ["documents", "metadatas", "distances"],
#             where            = where   # ← filter by source if specified
#         )
...
"""
embeddings.py  —  Final version
---------------------------------
Job : Embed chunks into vectors, store in ChromaDB, retrieve by similarity.
Fix : retrieve() now supports source_filter to search within a specific PDF only.
"""
...
def store_chunks(chunks: list) -> None:
    """
    Embed all chunks and store in ChromaDB.
    Skips chunks already stored — safe to re-run.
    """
    if not chunks:
        print("No chunks to store.")
        return
    ...
    print(f"\nEmbedding {len(new_chunks)} new chunks...")
    print("(This may take 1-3 minutes for large PDFs)")
...
def retrieve(question: str, top_k: int = 5, source_filter: str = None) -> list:
    ...
    Args:
        question      : plain English question
        top_k         : how many chunks to return (default 5)
        source_filter : if provided, only search within this PDF filename
                        e.g. "NestleAnnual-Report-2023-24.pdf"
...
if __name__ == "__main__":
    ...
    print("\nTest: search all documents")
    results = retrieve("Zomato revenue", top_k=3)
    for r in results:
        print(f"  {r['source']} | Page {r['page']} | Score {r['score']}")

    print("\nTest: search only Nestle")
    results = retrieve("revenue sales", top_k=3, source_filter="NestleAnnual-Report-2023-24.pdf")
    for r in results:
        print(f"  {r['source']} | Page {r['page']} | Score {r['score']}")

    print(f"\n✓ Embeddings working! {collection.count()} chunks in DB.")